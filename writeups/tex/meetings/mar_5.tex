...
\begin{align*}
    \frac{\partial C_0}{\partial w_{ij}^{(\ell)}} &= a_i^{(\ell)} \dot\sigma^{(\ell)}(z_i^{(\ell)})\cdot\frac{\partial C_0}{\partial a_i^{(\ell)}} \\
    \frac{\partial C_0}{\partial w_{ij}^{(\ell)}} &= \dot\sigma^{(\ell)}(z_i^{(\ell)})\cdot\frac{\partial C_0}{\partial a_i^{(\ell)}} \\
\end{align*}
...
\[
    \frac{\partial C_0}{\partial a_i^{(L)}} = 2(a_i^{(L)} - y_i)
\]
However, if we try to find an expression for the same derivative but in a previous layer, we find 
\[
    \frac{\partial C_0}{\partial a_i^{(\ell - 1)}} = \sum_{j = 1}^{n_{\ell}} \frac{\partial z_j^{(\ell)}}{\partial a_i^{(\ell - 1)}} \cdot 
...
\[
    \frac{\partial C_0}{\partial a_i^{(\ell)}} = \sum_{j = 1}^{n_\ell} w_{ij}^{(\ell)} \dot\sigma^{(\ell + 1)}(z_j^{(\ell + 1)}) \frac{\partial C_0}{\partial a_i^{(\ell + 1)}}
\]
Once these have been computed, one can directly compute
\[
    \frac{\partial C_0}{\partial w_{ij}^{(\ell)}} = a_i^{(\ell - 1)} \dot\sigma^{(\ell)}(z_i^{(\ell)}) \frac{\partial C_0}{\partial a_i^{(\ell)}} \;\;\; \text{and} \;\;\; \frac{\partial C_0}{\partial b_i^{(\ell)}} = \dot\sigma^{(\ell)}(z_i^{(\ell)}) \frac{\partial C_0}{\partial a_i^{(\ell)}}
\]

And that's it! This gives everything you need to fully compute $\nabla C_0$.

\textbf{\underline{Implementation Note:}} Perhaps a small speedup can be gained by noticing that 
\[
    \frac{\partial C_0}{\partial w_{ij}^{(\ell)}} = a_j^{(\ell - 1)}\frac{\partial C_0}{\partial b_i^{(\ell)}}
\]
which means you can just compute $\frac{\partial C_0}{\partial b_i^{(\ell)}}$ once instead of re-computing the same product.
